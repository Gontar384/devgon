...
describe('ProductService', () => {
    ...
    describe('create', () => {
        it('should call productRepository.create with correct params and return result', async () => {
            ...
            const result = await service.create(productData.title, productData.description);
        ...
        it('should call productRepository.create with title only when description is omitted', async () => {
            ...
            expect(repository.create).toHaveBeenCalledWith({ title: productData.title, description: undefined });
            expect(result).toEqual(createdProduct);
        });
    });
